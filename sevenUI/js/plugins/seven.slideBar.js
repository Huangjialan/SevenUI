...
Slider.prototype = {
	//容器对象，滑块
	initialize: function(container, bar, colorBar, options) {
		this.Bar = G(bar);
		this.Container = G(container);
		this.colorBar = G(colorBar);
		...
		//seven添加，根据data-position设置滑块默认的位置
		var percent = this.Container.dataset.position/100;
		if(this.Container.dataset.position > 0){
			var iLeft = (this.Container.clientWidth - this.Bar.offsetWidth) * percent;
			this.Bar.style.left = iLeft + "px";
			var barWidth = iLeft + 18 + "px";
			this.colorBar.style.width = barWidth;
		}
		...
		//点击控制
		addEventHandler(this.Container, "click", BindAsEventListener(this, function(e) {
			this._ondrag || this.ClickCtrl(e);
		}));
		//取消冒泡，防止跟Container的click冲突
		addEventHandler(this.Bar, "click", BindAsEventListener(this, function(e) {
			e.stopPropagation();
		}));
